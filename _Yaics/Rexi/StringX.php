<?php //*** StringX » Tydi™ Framework © 2024 ∞ AO™ • @osawereao • www.osawere.com ∞ Apache License ***//
namespace App\Spry;
...
class StringX
{
	...
	// • ==== hasNL • string has newline »
	public static function hasNL($str)
	{
	...
	// • ==== hasMultiNL • string has newline »
	public static function hasMultiNL($str)
	{
		if (preg_match('/(\R){2,}/', $str)) {
			return true;
		}
		return false;
	}
	...
	public static function beginWith($string, $begin, $strictCase = false)
	{
		if (self::isNotEmpty($string) && self::isNotEmpty($begin)) {
			$string = trim($string);

			if (!$strictCase) {
				return stripos($string, $begin) === 0;
			}
	...
	{
		if (self::beginWith($string, $needle, $strictCase)) {
	...
	{
		$string = self::toSentenceCase($string);
